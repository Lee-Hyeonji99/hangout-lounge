...
export function createMesh({ onPeerJoin, onPeerLeave, onData, peerId, iceServers } = {}) {
  ...
  /** 특정 한 명에게만 보낸다(미니게임 초대/수/진행 등 — docs/PLAN.md
   *  10번. broadcast로 방 전체에 뿌리면 게임 안 하는 사람들도 다 받게
   *  되니, 상대를 정확히 아는 메시지는 이걸로). 연결 안 된 상대면 조용히
   *  무시(호출하는 쪽에서 굳이 매번 존재 확인 안 해도 되게). */
